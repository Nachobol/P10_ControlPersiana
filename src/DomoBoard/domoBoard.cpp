...
void DomoBoard::SetGaraje(tsStaPer staPer)
{
	...
	case PER_STOP:
		...
		setActuator(&PER_ONOFF, false);								 // Desactiva la persiana (corta la alimentación o desactiva el relé)
		mbDomoboard.garajeWait.startWaiting(&Aregs[MB_TMP5_GARAJE]); // Configuramos el tiempo de espera al tiempo almacenado
	...
	case PER_STOP2:
		...
		setActuator(&PER_ONOFF, false);								 // Desactiva la persiana (corta la alimentación o desactiva el relé)
		setActuator(&PER_UPDOWN, false);							 // Dirección en estado bajo (por seguridad o neutralidad)
		mbDomoboard.garajeWait.startWaiting(&Aregs[MB_TMP1_GARAJE]); // Configuramos el tiempo de espera al tiempo almacenado
